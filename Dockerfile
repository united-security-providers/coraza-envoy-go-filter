...
FROM envoyproxy/envoy:contrib-v1.39.0 AS envoy
ARG BUILD_TAGS
...
RUN apt update && apt install -y golang-1.26-go
WORKDIR /src
COPY internal ./internal
COPY main.go go.mod go.sum .
RUN /usr/lib/go-1.26/bin/go build -o coraza-waf.so -buildmode=c-shared -tags=$BUILD_TAGS .
ENTRYPOINT ["/usr/bin/cp", "/src/coraza-waf.so", "/build"]

FROM envoyproxy/envoy:contrib-v1.39.0 AS envoy-coraza
