...
check_status "${envoy_url_echo}?arg=arg_1" 200 -H 'Host: foo.example.com'

# Testing filtered request
((step+=1))
echo "[${step}/${total_steps}] (onRequestheaders) Testing true positive custom rule"
check_status "${envoy_url_filtered}" 403 -H 'Host: foo.example.com'
...
check_body "${envoy_url_filtered}" true -H 'Host: foo.example.com'

# Testing body true negative
((step+=1))
echo "[${step}/${total_steps}] (onRequestBody) Testing true negative request (body)"
check_status "${envoy_url_post}" 200 -X POST -H 'Host: foo.example.com' -H 'Content-Type: application/x-www-form-urlencoded' --data "${tueNegativeBodyPayload}"

# Testing body detection
((step+=1))
echo "[${step}/${total_steps}] (onRequestBody) Testing true positive request (body)"
check_status "${envoy_url_post}" 403 -X POST -H 'Host: foo.example.com' -H 'Content-Type: application/x-www-form-urlencoded' --data "${truePositiveBodyPayload}"
...
check_status "${envoy_url_filtered_resp_header}" 403 -H 'Host: foo.example.com'

# Testing response body true negative
((step+=1))
echo "[${step}/${total_steps}] (onResponseBody) Testing true negative"
check_body "${envoy_url_post}" false -X POST -H 'Host: foo.example.com' -H 'Content-Type: application/x-www-form-urlencoded' --data "${trueNegativeBodyPayloadForResponseBody}"

# Testing response body detection
((step+=1))
echo "[${step}/${total_steps}] (onResponseBody) Testing true positive"
check_body "${envoy_url_post}" true -X POST -H 'Host: foo.example.com' -H 'Content-Type: application/x-www-form-urlencoded' --data "${truePositiveBodyPayloadForResponseBody}"
...
check_status "${envoy_url_post}" 403 -X POST -H 'Host: foo.example.com' -H 'Content-Type: application/x-www-form-urlencoded' --data "${truePositiveBodyPayloadForResponseBody}"
...
check_status "${envoy_url_echo}?arg=<script>alert(0)</script>" 403 -H 'Host: foo.example.com'

# Testing SQLI detection during phase 2
((step+=1))
echo "[${step}/${total_steps}] Testing SQLi detection at request body"
check_status "${envoy_url_post}" 403 -H 'Host: foo.example.com' -X POST --data "1%27%20ORDER%20BY%203--%2B"

# Triggers a CRS scanner detection rule (913100)
((step+=1))
echo "[${step}/${total_steps}] (onRequestBody) Testing CRS rule 913100"
check_status "${envoy_url_echo}" 403 -H 'Host: foo.example.com' --user-agent "gobuster/3.2.0 (X11; U; Linux i686; en-US; rv:1.7)" -H "Host: localhost" -H "Accept: text/xml,application/xml,application/xhtml+xml,text/html;q=0.9,text/plain;q=0.8,image/png,*/*;q=0.5"

# True negative GET request with an usual user-agent
((step+=1))
echo "[${step}/${total_steps}] True negative GET request with user-agent"
check_status "${envoy_url_echo}" 200 -H 'Host: foo.example.com' --user-agent "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/105.0.0.0 Safari/537.36"


## Testing per route configuration
((step+=1))
echo "[${step}/${total_steps}] Testing per route config true negative"
check_status "${envoy_url_unfiltered}/other-waf" 404 -H 'Host: foo.example.com'
...
check_status "${envoy_url_unfiltered}/other-waf/other-admin" 403 -H 'Host: foo.example.com'

((step+=1))
echo "[${step}/${total_steps}] Testing per route true positive request body"
check_status "${envoy_url_unfiltered}/other-waf" 403 -X POST -H 'Host: foo.example.com' -H 'Content-Type: application/x-www-form-urlencoded' --data "evilpayload"

# Testing per virtual host configuration
# we use the sse-server /health endpoint
((step+=1))
echo "[${step}/${total_steps}] Testing per virtual host config true negative"
check_status "${envoy_url_unfiltered}/health" 200 -H 'Host: foo.vhost-example.com'

((step+=1))
echo "[${step}/${total_steps}] Testing other wafs do not block per virtual host config true negative"
check_status "${envoy_url_unfiltered}/health/admin" 404 -H 'Host: foo.vhost-example.com'

((step+=1))
echo "[${step}/${total_steps}] Testing per virtual host config true positive"
check_status "${envoy_url_unfiltered}/health/vhost-admin" 403 -H 'Host: foo.vhost-example.com'
